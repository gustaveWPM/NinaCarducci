  ...
  function injectMau(props) {
    ...
    function moveCameraToSavedPosition(activeElement = null) {
      ...
      if (activeElement && !isOnMobile()) {
        ...
        const currentComputedElementBottom = parseInt(window.getComputedStyle(activeElement).bottom, 10);
        const distance = memos('activeElementComputedBottom') - currentComputedElementBottom;
        const absDistance = Math.abs(memos('activeElementComputedBottom') - currentComputedElementBottom);
        const currentComputedHeight = parseInt(window.getComputedStyle(activeElement).height, 10);

        if (absDistance >= currentComputedHeight) {
          if (distance < 0) {
            activeElement.scrollIntoView(false);
          } else if (distance > 0) {
            activeElement.scrollIntoView(true);
    ...
    function generateListeners(gallery, modal) {
      ...
      function handleSwipe() {
        ...
        const approxZoom = (( window.outerWidth - 10 ) / window.innerWidth) * 100;
        if (window.innerHeight > window.innerWidth) {
          if (Math.trunc(approxZoom) > 100) {
            return;
          }
        }
        if (window.innerHeight < window.innerWidth) {
          if (Math.trunc(approxZoom) > 110) {
            return;
          }
        }
    ...
    function createLightBox(gallery) {
      ...
      const lightbox = `
        <div class="${mauPrefixClass} modal fade" id="${lightboxId}" tabindex="-1" role="dialog" aria-hidden="true" style="display:none;align-items:center;justify-content:center;user-select:none;-webkit-user-select:none;">
          <div class="modal-dialog" role="document" ${dynamicModalDialogWidth}>
