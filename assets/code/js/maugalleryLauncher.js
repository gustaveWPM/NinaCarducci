        ...
        if (pkg.options.injectionProperties['alreadyAsyncLoadedSomewhereElse']) {
          ...
          for (let retryCounter = 1; retryCounter <= max; retryCounter++) {
            ...
            if (!error) {
              ...
              me.debugger(
                `Failed to fetch ${pkg['name']}. Retrying (${retryCounter}/${pkg.options.injectionProperties['maximumPackageFetchRetry']})`
              );
            }
          }
    ...
    constructor(name, url, requiredFeatures, opt = {}) {
      ...
      if (!this.options.injectionProperties['checkRequiredFeaturesTimeout']) {
        this.options.injectionProperties['checkRequiredFeaturesTimeout'] =
          _asyncMauGalleryLauncher.Launcher_Instance.launcherConfig['defaultCheckRequiredFeaturesTimeout'];
      }

      if (!this.options.injectionProperties['maximumPackageFetchRetry']) {
        this.options.injectionProperties['maximumPackageFetchRetry'] =
          _asyncMauGalleryLauncher.Launcher_Instance.launcherConfig['defaultMaximumPackageFetchRetry'];
      }

      if (!this.options.injectionProperties['retryFetchPackageDelay']) {
        this.options.injectionProperties['retryFetchPackageDelay'] =
          _asyncMauGalleryLauncher.Launcher_Instance.launcherConfig['defaultRetryFetchPackageDelay'];
      }
